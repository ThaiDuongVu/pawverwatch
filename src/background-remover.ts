...
export const initializeModel = async (useWebGPU=true): Promise<boolean> => {
  try {
    ...
    const selectedModelId = useWebGPU ? WEBGPU_MODEL_ID : FALLBACK_MODEL_ID;
...
export const removeBackground = async (image: File | null): Promise<File> => {
  if (!state.model || !state.processor) {
